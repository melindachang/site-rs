...
possible to make quality-of-life enhancements to various websites with
...
the base functionality I want (that is, programmatic stylesheet
injections) with minimal friction. It needs to support the following:
...
2. _Modern frameworks, TypeScript, and CSS preprocessing._ I use SCSS,
   mostly, because I find that indented syntax sometimes interferes
   with formatters. I haven't written vanilla JavaScript or CSS since
...
export default defineContentScript({
  ...
  async main() {
    const settings = await getUserSettings() // { darkMode: bool; test: bool }
